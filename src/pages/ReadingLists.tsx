...
export function ReadingLists() {
  ...
  const loadLists = async (userId: string) => {
    console.log("ReadingLists userId:", userId);

    setIsLoading(true);
    try {
      const data = await getReadingLists(userId);
      ...
      <div className="container mx-auto">
        ...
        {!user?.id ? (
          <div className="text-center py-12 bg-white/90 backdrop-blur-sm rounded-xl shadow-lg border border-slate-200">
            ...
            <p className="text-slate-600">You need to be logged in to view your reading lists.</p>
          </div>
        ) : lists.length === 0 ? (
          <div className="text-center py-12 bg-white/90 backdrop-blur-sm rounded-xl shadow-lg border border-slate-200">
            <svg
